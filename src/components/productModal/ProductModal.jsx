        ...
        >
          <div className="modal_card_products">
            <div className="modal_product_img">
              <img src={modalInfo.image} alt="product-image" />
            </div>
                ...
                <div className="food_category">
                  ...
                  <ol>
                    {modalInfo?.ingredients?.length > 0 &&
                      modalInfo.ingredients?.map((category, index) => {
                        return <li key={index}>{category.foodCategory}</li>;
                      })}
                  </ol>
                </div>
                <div className="food_ingredients">
                  <h3>Ingredients:</h3>
                  {modalInfo?.ingredientLines?.length > 0 &&
                    modalInfo.ingredientLines.map((ingre, index) => {
                      return <span key={index}>{ingre}</span>;
                    })}
                </div>
              </div>
              <div className="add_to_card">
                <button onClick={addFoodCloseModal}>
                  Добавить в корзину за{" "}
                  {new Intl.NumberFormat("en-US", { style: "decimal" }).format(
                    modalInfo.totalWeight
                  )}{" "}
                  сум
                </button>
              </div>
